...
const BookingOrderManagement = () => {
  ...
  const fetchTables = async (page = 1, showLoading = true) => {
    try {
      ...
      const response = await axios.get(
        `${backendApiUrl}/reservation/staff?page=${page}&limit=${limit}`
      );
  ...
  const handleApplyFilter = async (page = 1, customFilters = filters) => {
    try {
      ...
      if (customFilters.dateRange)
        queryParams.append("dateRange", customFilters.dateRange);
      if (customFilters.fromDate)
        queryParams.append("fromDate", customFilters.fromDate);
      if (customFilters.toDate)
        queryParams.append("toDate", customFilters.toDate);
      if (customFilters.orderType)
        queryParams.append("orderType", customFilters.orderType);
      if (customFilters.status)
        queryParams.append("status", customFilters.status);
      if (customFilters.searchText.trim())
        queryParams.append("searchText", customFilters.searchText.trim());

      const response = await axios.get(
        `${backendApiUrl}/reservation/filter?${queryParams.toString()}`
      );
      const { data, totalPages } = response.data;
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    try {
      await axios.put(`${backendApiUrl}/reservation/${id}/status`, {
        status: newStatus,
      });
  ...
  return (
    <div className="mx-auto bg-white p-6 rounded-lg shadow-md flex flex-col min-h-[568px]">
      <div className="flex-wrap items-center gap-3 mb-2 flex justify-between">
      ...
      <div className="relative overflow-x-auto scrollbar-hide">
        {loading && (
          <div className=" inset-0 bg-white bg-opacity-60 z-50 flex items-center justify-center relative h-[63vh]">
            <LoadingAnimation />
          </div>
        )}
                ...
                <tbody>
                  {bookings.map((booking, index) => (
                    <tr key={booking._id} className="border-b">
                      ...
                      <td className="p-4">
                        <span
                          className={`inline-block px-3 py-1 rounded-full font-bold text-sm capitalize min-w-[90px] text-center ${
                            booking.orderType.toLowerCase() === "dine-in"
                              ? "text-green-700"
                              : booking.orderType.toLowerCase() === "delivery"
                              ? "text-blue-800"
                              : "text-gray-600"
                          }`}
                        >
                      ...
                      <td className="p-4 whitespace-nowrap">
                        {isStatusLocked(booking.status) ? (
                          <span
                            ...
                            style={{
                              backgroundColor:
                                getStatus(booking.status) === "Confirmed"
                                  ? "#d4edda"
                                  : getStatus(booking.status) === "Pending"
                                  ? "#fff3cd"
                                  : getStatus(booking.status) === "Canceled"
                                  ? "#fee2e2"
                                  : getStatus(booking.status) === "Cooking"
                                  ? "#ffe0b2"
                                  : getStatus(booking.status) === "Ready"
                                  ? "#dbeafe"
                                  : "#ffcc80",
                              color:
                                getStatus(booking.status) === "Confirmed"
                          ...
                          >
                            ...
                            {getValidNextStatuses(booking.status).map(
                              (statusOpt) => (
                  ...
                  >
                    ...
                    <div className="mb-2 flex items-center gap-2 whitespace-nowrap">
                      ...
                      <div className="mt-1 ">
                        {isStatusLocked(booking.status) ? (
                          <span
                            className="inline-flex w-[121px] items-center justify-center gap-2 px-3 py-1 rounded-full font-bold text-sm text-center"
                            style={{
                              backgroundColor:
                                getStatus(booking.status) === "Confirmed"
                                  ? "#d4edda"
        ...
        <div className="flex flex-col gap-4 text-base">
          ...
          {selectedBooking ? (
            <>
                ...
                <div className="flex justify-between border-b border-dashed py-2 text-base">
                  ...
                  <span>
                    {selectedBooking.userId?.phoneNumber ||
                      selectedBooking.guest?.contactPhone ||
                      "N/A"}
                  </span>
                </div>
                <div className="flex justify-between border-b border-dashed py-2 text-base">
                    ...
                    {selectedBooking.tableId && (
                      <div className="flex justify-between border-b border-dashed py-2 text-base">
                        ...
                        <span className="text-gray-700">
                          {selectedBooking.tableId.capacity}
                        </span>
                      </div>
                    )}
                  </>
              ...
              {Array.isArray(selectedBooking.dishes) &&
              selectedBooking.dishes.length > 0 ? (
              ...
              <div className="flex justify-between border-b border-dashed py-2 text-base">
                ...
                <span
                  ...
                  style={
                    statusModelStyles[getStatus(selectedBooking.status)] ||
                    statusModelStyles.Unknown
                  }
                >
